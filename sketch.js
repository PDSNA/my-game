// 최종 수정 코드 (2025-08-01) - PC/모바일 완벽 지원
let player;
let customer;
...
class Ingredient {
    ...
    // ★★★★★ 모바일/PC 모두 지원하도록 수정된 isMouseOver
    isMouseOver(checkX = mouseX, checkY = mouseY) {
...
class Player {
    ...
    // ★★★★★ 드래그(touchMoved)가 반영되도록 수정
    move() {
        this.x = mouseX; // PC 기본값
        if (mouseIsPressed || touches.length > 0) {
            this.x = touches.length > 0 ? touches[0].x : mouseX;
        }
...
// ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
// ★★★★★ 코드 수정 부분 (PC/모바일 입력 처리) ★★★★★
// ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼

// 모든 클릭/터치 로직을 처리하는 중앙 함수
function handleInteraction(x, y) {
    if (!gameStarted) {
        if (showTutorialScreen) {
            if (backToMenuButton.isMouseOver(x,y)) {
                showTutorialScreen = false;
                difficultySelected = false;
            }
        } else {
            if (!difficultySelected) {
                if (normalModeButton.isMouseOver(x,y)) {
                    gameDifficulty = 'normal';
                    difficultySelected = true;
                    startGameButton = new Button("START GAME", width / 2 - 100, height / 2 + 90, 200, 50, color(50, 200, 50));
                } else if (hardModeButton.isMouseOver(x,y)) {
                    gameDifficulty = 'hard';
                    difficultySelected = true;
                    startGameButton = new Button("START GAME", width / 2 - 100, height / 2 + 90, 200, 50, color(50, 200, 50));
                } else if (tutorialButton.isMouseOver(x,y)) {
                    showTutorialScreen = true;
                }
            } else {
                if (startGameButton && startGameButton.isMouseOver(x,y)) {
                    startGame(gameDifficulty);
                }
            }
        }
    } else if (gameOver) {
        if (restartGameButton && restartGameButton.isMouseOver(x,y)) {
            restartGame();
        } else if (goToMenuButton && goToMenuButton.isMouseOver(x,y)) {
            goToMainMenu();
        } else if (clearRankingButton && clearRankingButton.isMouseOver(x,y)) {
            clearRanking();
        }
    } else { // 게임 진행 중
        if (annoyingCustomer && annoyingCustomer.isMouseOver(x,y)) {
        ...
        if (x > trash.x && x < trash.x + trash.width && y > trash.y && y < trash.y + trash.height) {
            currentDrink = [];
            player.holdingDrink = false;
            player.currentReadyDrink = null;
        }
...
function touchStarted() {
    ...
    return false; // 브라우저 기본 동작(줌 등) 방지
}

// 모바일에서 드래그(플레이어 이동)를 처리
function touchMoved() {
    if (gameStarted && !gameOver) {
        player.x = touches[0].x;
        player.x = constrain(player.x, player.w / 2, width - player.w / 2);
    }
    return false; // 브라우저 기본 스크롤 동작 방지
}