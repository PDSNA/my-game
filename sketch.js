...
function drawTrashCanButton() {
    ...
    fill(checkX > x && checkX < x + w && checkY > y && checkY < y + h ? color(180, 180, 180) : color(150, 150, 150));
    stroke(0);
    strokeWeight(1);
...
// ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
// ★★★★★ 코드 수정 부분 ★★★★★
// 기존 mousePressed() 함수를 아래의 3개 함수로 교체합니다.
// ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼

// 1. 모든 클릭/터치 로직을 담을 새로운 함수를 만듭니다.
function handleInteraction() {
    if (!gameStarted) {
        ...
        // 쓰레기통 터치 로직
        let resetButton = {x: resetButtonPos.x, y: resetButtonPos.y, w: resetButtonPos.width, h: resetButtonPos.height};
        let checkX = mouseX;
        let checkY = mouseY;
        if (touches.length > 0) {
            checkX = touches[0].x;
        ...
        if (checkX > resetButton.x && checkX < resetButton.x + resetButton.w &&
            checkY > resetButton.y && checkY < resetButton.y + resetButton.h) {
            currentDrink = [];
            player.holdingDrink = false;
            player.currentReadyDrink = null;
        }
...
// 2. mousePressed는 이제 handleInteraction만 호출합니다.
function mousePressed() {
    handleInteraction();
}

// 3. touchStarted는 handleInteraction을 호출하고, 모바일 브라우저의 기본 동작을 막습니다.
function touchStarted() {
    handleInteraction();
    return false; // 이 부분이 중요합니다!
}```