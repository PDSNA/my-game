...
// ... Customer, AnnoyingCustomer, VIPCustomer 클래스는 그대로 ...
class Customer {
    constructor() {
        this.x = baseWidth - 80;
...
// ... 나머지 모든 함수(showStartScreen, showGameOverScreen 등)는 그대로 ...
...
function drawTrashCanButton() {
    ...
    let isHover = this.isMouseOver(mouseX / scaleFactor, mouseY / scaleFactor, x, y, w, h);
...
function handleInteraction(x, y) {
    ...
    if (!gameStarted) {
        ...
        if (gameX > trash.x && gameX < trash.x + trash.width && gameY > trash.y && gameY < trash.y + trash.height) {
            currentDrink = [];
            player.holdingDrink = false;
            player.currentReadyDrink = null;
        }
